...
- 浏览器界面模型由 `RUOYI-AUTH-GATEWAY-001`（2026-08-20 立项）变更为若依统一登录门：所有人必须先经若依账号登录，`普通用户角色` 进入任务页、`developer/admin` 角色进入开发者控制台，业务数据按账号隔离；现有"免登录普通任务页 + 环境变量开发者密码"模型在该任务 Phase 3/4 落地时退役，落地前代码仍为旧模型，进度以 `.ai-team/TASK.md` 为准。
- 默认仅监听回环地址，PC 开发全程保持回环；公网访问只能经若依认证网关 + HTTPS 反向代理（`RUOYI-AUTH-GATEWAY-001` 建设中）提供；受信任开发网络预览的"非回环 + 开发者密码"要求在网关落地后由若依 developer 角色替代。
...
- 普通浏览器会话和开发者会话均由服务端生成的 HttpOnly Cookie 标识；调用者提供的角色或会话请求头不是授权事实。目标态（`RUOYI-AUTH-GATEWAY-001` Phase 3 起）：Cookie 会话建立在若依 JWT 验签 + Redis 会话校验通过之后，业务数据按账号隔离。
- 开发者密码来自仓库外环境变量，登录令牌仅保存在服务端内存中；服务重启会使开发者登录失效。该密码门在若依角色判定落地（Phase 3）后退役。
...
- 现行：未登录用户只能访问自己的普通任务能力；管理设置、完整能力信息、任务审计、日志和 API 文档必须要求有效开发者会话。目标态（`RUOYI-AUTH-GATEWAY-001` Phase 3 起）：一切 API 访问必须持有效若依令牌（白名单仅 `/health`），管理、审计、日志和文档再要求 developer/admin 角色。
- 非回环监听地址必须同时配置开发者密码（若依角色判定落地后由角色替代）；该门禁只支持受信任开发网络预览，公网形态只允许经若依认证网关 + HTTPS 反向代理，不替代生产认证。
