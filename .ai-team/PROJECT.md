...
- 浏览器界面采用若依统一登录门（`RUOYI-AUTH-GATEWAY-001` Phase 3 于 2026-08-21 落地，ADR 0008）：所有人必须先经若依账号登录，`普通用户角色` 进入任务页、`developer/admin` 角色进入开发者控制台，业务数据按账号（userId）隔离；旧"免登录普通任务页 + 环境变量开发者密码"模型已退役（ADR 0007 被取代）。
- 默认仅监听回环地址，PC 开发全程保持回环；公网访问只能经若依认证网关 + HTTPS 反向代理（`RUOYI-AUTH-GATEWAY-001`，Phase 4～6 建设中）提供。
...
- 未经若依认证网关与 HTTPS 反向代理把 API 暴露到公网，或把开发板局域网预览能力宣称为生产级网络安全方案；经认证网关的公网访问形态由 `RUOYI-AUTH-GATEWAY-001` 建设，不属于本条禁止范围。
...
- 请求身份由 RuoYi JWT 闸机中间件验证（契约 `docs/contracts/ruoyi-auth-gateway.md`）：HS512 验签 + 只读查若依 Redis `login_tokens:{uuid}` 会话；`request.state.role`（user/developer）与 `request.state.session_id`（`user:{userId}`，数据归属键）只来自已验证令牌，调用者提供的角色或会话请求头不是授权事实。
- 登录与登出发生在若依侧；agent_platform 不保存任何密码或登录会话。业务数据（任务、待办、日程、提醒、知识库、语音记录）按 owner=userId 隔离，工具幂等缓存键同按 owner 命名空间化。
...
- 一切 API 访问必须持有效若依令牌（白名单仅 `GET /health` 与静态页面资源，契约 §5）；未认证一律真 HTTP 401（稳定码 `auth_required`），Redis 不可达时 fail-closed。管理设置、完整能力信息、任务审计、日志和 API 文档再要求 developer/admin 角色。
- 非回环监听不再需要开发者密码（该门已随 ADR 0007 退役）；认证由若依令牌闸机承担，不依赖绑定地址。公网形态仍只允许经若依认证网关 + HTTPS 反向代理，不替代生产认证。
