...
async def test_schedule_invalid_time_and_reversed_end_do_not_write(tmp_path, monkeypatch):
    tool = ScheduleTool(tmp_path / "schedules.db")
    _clocked(tool, monkeypatch)
    invalid = await tool.execute({"action": "create", "title": "不完整", "start_text": "下周开会"})
...
async def test_schedule_persists_after_reopen(tmp_path):
    ...
    created = await tool.execute({"action": "create", "title": "持久化", "start_text": "2026-07-28 14:00"})
    schedule_id = created.output["item"]["id"]
    tool.close()
    reopened = ScheduleTool(path)
    queried = await reopened.query({"action": "query", **_custom(NOW, NOW + timedelta(days=1))}, now=NOW)
...
async def test_schedule_daily_recurrence_is_bounded_to_query_range(tmp_path, monkeypatch):
    tool = ScheduleTool(tmp_path / "schedules.db")
    _clocked(tool, monkeypatch)
    await tool.execute({"action": "create", "title": "日报", "start_text": "每天下午3点", "recurrence": "daily"})
    queried = await tool.query({"action": "query", **_custom(NOW, NOW + timedelta(days=4))}, now=NOW)
...
async def test_schedule_weekly_multiple_weekdays_expand_correctly(tmp_path, monkeypatch):
    ...
    queried = await tool.query({"action": "query", **_custom(NOW, NOW + timedelta(days=14))}, now=NOW)
...
async def test_schedule_monthly_uses_original_day_and_skips_invalid_month_days(tmp_path, monkeypatch):
    tool = ScheduleTool(tmp_path / "schedules.db")
    _clocked(tool, monkeypatch)
    await tool.execute({"action": "create", "title": "月度", "start_text": "每月31日 10点", "recurrence": "monthly"})
    start = datetime(2026, 8, 1, tzinfo=TZ)
    end = datetime(2026, 9, 1, tzinfo=TZ)
    queried = await tool.query({"action": "query", **_custom(start, end)}, now=NOW)
...
async def test_schedule_recurrence_until_is_an_inclusive_upper_boundary(tmp_path, monkeypatch):
    ...
    queried = await tool.query({"action": "query", **_custom(NOW, NOW + timedelta(days=5))}, now=NOW)
...
async def test_schedule_named_ranges_today_tomorrow_and_weeks(tmp_path):
    tool = ScheduleTool(tmp_path / "schedules.db")
    await tool.execute({"action": "create", "title": "今天", "start_text": "2026-07-28 14:00"})
    await tool.execute({"action": "create", "title": "明天", "start_text": "2026-07-29 14:00"})
    today = await tool.query({"action": "query", "range": "today"}, now=NOW)
    tomorrow = await tool.query({"action": "query", "range": "tomorrow"}, now=NOW)
    this_week = await tool.query({"action": "query", "range": "this_week"}, now=NOW)
    next_week = await tool.query({"action": "query", "range": "next_week"}, now=NOW)
...
async def test_schedule_rejects_custom_ranges_over_31_days_without_side_effect(tmp_path):
    tool = ScheduleTool(tmp_path / "schedules.db")
    result = await tool.query({"action": "query", **_custom(NOW, NOW + timedelta(days=32))}, now=NOW)
...
async def test_schedule_cancel_marks_only_target_and_unknown_id_is_safe(tmp_path):
    tool = ScheduleTool(tmp_path / "schedules.db")
    first = await tool.execute({"action": "create", "title": "同名", "start_text": "2026-07-28 14:00"})
    second = await tool.execute({"action": "create", "title": "同名", "start_text": "2026-07-29 14:00"})
    first_id = first.output["item"]["id"]
    second_id = second.output["item"]["id"]
    cancelled = await tool.execute({"action": "cancel", "id": first_id})
    assert cancelled.output["item"]["status"] == "cancelled"
    assert (await tool.execute({"action": "cancel", "id": 9999})).output["updated"] is False
    remaining = await tool.query({"action": "query", **_custom(NOW, NOW + timedelta(days=2))}, now=NOW)
