...
class Settings(BaseSettings):
    ...
    # topology where RuoYi is the site root behind the reverse proxy), which
    # same-origin path revokes the RuoYi session on console logout (Nginx
    # default "/prod-api/logout"; the Vue dev-server proxy uses "/dev-api/logout"),
    # and where the developer console's management backoffice link points
    # (empty = hide the link; the Nginx topology uses "/index").
    ...
    ruoyi_manage_url: str = Field(default="", validation_alias="RUOYI_MANAGE_URL")
