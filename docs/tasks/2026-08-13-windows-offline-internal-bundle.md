...
| 验证项 | 结果 | 说明 |
|---|---:|---|
| Self-Check | 22/22 通过 | 0 失败；另有 1 条预期的“不可分发本机验证包”警告 |
...
最终源码提交为 `a08a51a17a6015daa6812d172b0ef6ea4ab9d52b`。干净构建目录包含 `4,699` 个文件，总大小 `3,851,008,890` bytes；其中初始负载清单记录 `4,696` 个文件、`3,849,540,929` bytes。最长相对路径为 108 字符；深路径隔离目录的最长完整路径为 235 字符。冒烟产生的日志和 WAV 位于隔离副本，不进入干净归档。

Zip64 本机验证归档名为 `cloud-flowing-windows-x64-offline-local-validation.zip`，大小 `3,636,131,116` bytes，SHA256 为 `d72d50291efacc1ffb86709de7367e06bce10ab9d160cc8aa372e05da6f8b92e`。ZIP 完整性检查覆盖 `4,699` 个条目且无损坏项；最终独立解压目录复核 `SHA256SUMS` 共 `4,694` 条、0 错误，自检 22/22 通过。运行与自检后仍为 0 个 `.pyc`。
